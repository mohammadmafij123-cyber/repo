...
# 5. Dashboard Terminal Screen (সবগুলো অপশন সহজ ইনডেন্টে সাজানো)
if menu == "🏠 Execution Terminal":
    ...
    with left_layout:
        ...
        fig = go.Figure(data=[go.Candlestick(x=dates, open=opens, high=highs, low=lows, close=closes, increasing_line_color='#02c076', decreasing_line_color='#f6465d', increasing_fillcolor='#02c076', decreasing_fillcolor='#f6465d', line=dict(width=1.0))])
        fig.update_layout(plot_bgcolor='#161a1e', paper_bgcolor='#0b0e11', xaxis_rangeslider_visible=False, height=380, margin=dict(t=10, b=10, l=10, r=10), xaxis=dict(showgrid=True, gridcolor='#24292e', type='date', range=[dates[-30], dates[-1]]), yaxis=dict(showgrid=True, gridcolor='#24292e', side='right'))
